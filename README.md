...
The dashboard needs a few secrets. They are **not** in git — create them once.

**Local development:** create a file named `.env.local` in the project root:

```
ADMIN_EMAIL=pick-any-email-for-local
...
`ADMIN_EMAIL` / `ADMIN_PASSWORD` pair you chose. Restart the dev server after
editing `.env.local`.
